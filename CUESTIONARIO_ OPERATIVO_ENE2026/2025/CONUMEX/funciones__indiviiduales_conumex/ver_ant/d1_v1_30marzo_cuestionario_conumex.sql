...
*/
    ...
    (select canal,count(*) numero_operaciones,sum(monto_operaciones) monto_operaciones,flujo_entrada_salida 
                 from (select (case when ad.idtipo = 3 and ad.cargoabono=0 and (ad.concepto like 'Transferencia de banca movil%' 
                                                            or ad.concepto like 'Transferencia de banca movil%') then 5 /*Banca Móvil*/
                       ...
                       select (case when rc.idtipo = 3 and rc.cargoabono=1 and (rc.concepto like 'Transferencia de banca movil%' 
                                                                                or rc.concepto like 'Transferencia de banca movil%') then 5 /*Banca Móvil*/
                                                        else 1 /*Ventanilla (sucursales, agencias u oficinas)*/ end) as canal,
                                                  -- GUS (round)
                                           ...
                                           --- la tabla para banca mobil es temp_auxi_bk 
                                           --inner join polizas as po using (idorigenc, periodo, idtipo, idpoliza)
