...
-----MANDA A TRAER EL USUARIO QUE TIENEN EN BANCA MOVIL
...
----- TABLA DE PRODCUTOS POR SUCURSAL -----
...
*/
    ...
    IF (r_prod.dato2 is not null and TRIM(r_prod.dato2) <> '' or r_prod.idelemento::integer=27) THEN 
    --****Agregué el or para que permita ingresar el prod 27 (ordenes de pagos nacionales)****

    for r_paso in 
    (select canal,count(*) numero_operaciones,sum(monto_operaciones) monto_operaciones,flujo_entrada_salida 
                 from (select (case when ad.idtipo = 3 
                                    and ad.cargoabono=0 
                                    and (ad.concepto like 'Transferencia de banca movil%' 
                                    or ad.concepto like 'Transferencia de banca movil%') then 6 /*Banca Móvil*/
                                    else 1 end) as canal,/* 1 Ventanilla (sucursales, agencias u oficinas)*/ 
                       ...
                       select (case when rc.idtipo = 3 
                                    and rc.cargoabono=1 
                                    and (rc.concepto like 'Transferencia de banca movil%' 
                                    or rc.concepto like 'Transferencia de banca movil%') then 6 /*Banca Móvil*/
                       else 1 /*Ventanilla (sucursales, agencias u oficinas)*/ end) as canal,
                       -- GUS (round)
                       round((rc.monto + rc.montoio + rc.montoim + rc.montoiva + rc.montoivaim),0) as monto_operaciones,
                       
                       (case when rc.cargoabono=0 then 2
                             when rc.cargoabono=1 then 1 
                       end) as flujo_entrada_salida
                       from temp_auxi_bk rc
                       inner join polizas as po using (idorigenc, periodo, idtipo, idpoliza)
