...
RAISE NOTICE 'Se creo a2_cuestionario_operatividad';
  ...
  CREATE temp table copiar(
      ...
      fila  text);
  y:=0;
  insert into copiar values(y,'ANIO|CLAVE DE FORMULARIO|CLAVE DE LA ENTIDAD|TIPO DE PRODUCTO O SERVICIO|TIPO DE CLIENTE O USUARIO|CLASIFICACION POR GRADO DE RIESGO|PAIS NACIONALIDAD| PAIS RESIDENCIA|ENTIDAD FEDERATIVA|ACTIVIDAD ECONOMICA|NUMERO DE CLIENTES O  USUARIOS ACTIVOS AL CIERRE');
  y:=1;

---PAIS DE NACIONALIDAD|PAIS DE RESIDENCIA|ENTIDAD FEDERATIVA DE RESIDENCIA|ACTIVIDAD ECONOMICA|
...
/*INICIA QUERY PARA LA OBTENCION DE DATOS, LOS DATOS 1-3 SE LLENAN AL EJECUTAR LA FUNCION*/
  for r_prod in
    select * from tablas
  ...
  loop
    ...
    for r_paso in
      ...
      from (select count(*) as num_cli, tipoc, nries,nacion ,entidad_federativa_residencia,actividad_economica
            ...
            ,nacion,entidad_federativa_residencia,actividad_economica
            order by tipoc) aa
    loop
