...
-----SE AGREGARON JOINS PARA SU GENERACION EN LAS BASES DIC 2025 DE LAS COOPERATIVAS 
                        ...
                        inner  join trabajo tr on a.idorigen = tr.idorigen and a.idgrupo = tr.idgrupo and a.idsocio = tr.idsocio AND tr.consecutivo = 1
...
INSERT INTO  a2_cuestionario_operatividad  VALUES (
 ...
 r.numero_clientes_usuarios
    ...
    execute 'copy (select fila from copiar order by id) to ''/tmp/formularioA2_con_encabezados_'||fecha||'.csv'' with csv DELIMITER ''-'' ';
    execute 'copy (select fila from copiar where id > 0 order by id) to ''/tmp/formularioA2_sin_encabezados_'||fecha||'.csv'' with csv DELIMITER ''-'' ';
