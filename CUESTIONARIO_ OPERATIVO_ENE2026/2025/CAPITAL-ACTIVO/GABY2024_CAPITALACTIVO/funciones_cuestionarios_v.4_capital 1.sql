...
------------------------------------------------------------------------------------------------------------------------
 ...
 idsocio             integer
  ...
  loop
    ...
    insert into tmp_act
        select distinct idorigen,idgrupo,idsocio from auxiliares where idproducto = rec.dato1::integer and saldo  >=  rec.dato2::numeric;    
  end loop;
...
for r_paso in select * from
        (select tipo_clien_usua,clasi_grado_riesgo,enti_fede_residencia,sum(num_total_clien_usua) as num_total_clien_usua,espep,nacion from
        (select
        ...
        from personas p
        left join tmp_act_peps ac using (idorigen, idgrupo, idsocio)
        left join (select distinct idorigen,idgrupo,idsocio from tmp_act) as ax using (idorigen,idgrupo,idsocio)
        ...
        group by idorigen, idgrupo, idsocio,p.nacionalidad,p.razon_social,p.nivel_riesgo,e.idestado, peps, p.pais_nacimiento) ope
        group by tipo_clien_usua,clasi_grado_riesgo,enti_fede_residencia, espep, nacion) aa
loop
...
begin
  ...
  insert
  into   copiar
  values (y,'anio;clave_formulario;clave_entidad;producto_servicio;tipo_moneda;tipo_instrumento_monetario;operac_entr_salida;numero_operaciones;monto_operaciones');
    ...
    (select canal,count(*) numero_operaciones,sum(monto_operaciones) monto_operaciones,flujo_entrada_salida 
                 from (select (case when ad.idtipo = 3 and ad.cargoabono=1 and (ad.concepto = 'Traspaso Cuentas Propias Mitras Movil' 
                       ...
                       select (case when rc.idtipo = 3 and rc.cargoabono=1 and (rc.concepto = 'Traspaso Cuentas Propias Mitras Movil' 
                                                  ...
                                                  (rc.monto + rc.montoiva) as monto_operaciones,
