...
/*	drop table if exists temp_productos;
	create temp table temp_productos (idcuestionario integer, idproducto integer);
		FOR rec in  select * from tablas
					where idtabla = 'cuestionario_operativo_cnbv'
	...
	create temp table temp_auxi as 
		...
		select a.*, te.idcuestionario
		 ...
		 where a.idproducto=101 and a.saldo=500 and to_char(date(a.fechaape),'YYYYMM'):: integer between p_inicial and p_final
		 UNION
		 select a.*, te.idcuestionario
		 from auxiliares_h a
		 inner join temp_productos te using (idproducto)
	...
	select into r_paso count(*) as total_cli
	from (select distinct ta.idorigen,ta.idgrupo,ta.idsocio
		  from temp_auxi ta) as a;*/
		...
		select into r_paso sum(ts.total) as total_cli
			...
			and ad.monto=500 
			and ad.cargoabono=1 
			and to_char(date(ad.fecha),'YYYYMM'):: integer between p_inicial and p_final 
			and ad.idtipo=1
		union
			select count(*) as total
