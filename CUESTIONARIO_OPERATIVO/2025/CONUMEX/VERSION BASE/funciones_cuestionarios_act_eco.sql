...
---------- CREACION DE LA TABLA PARA SOCIOS QUE APLICAN LA VALIDACION
      ...
      from (select count(*) as num_cli, tipoc, nries,nacion ,entidad_federativa_residencia,actividad_economica
            from (select /* (case when p.nacionalidad != 3 and (p.razon_social is NULL or p.razon_social = '') then 1
                  ...
                  (case when a.peps = 1 then 15
                            ...
                            when (select count(*) from temp_pfae
                            where idorigen = p.idorigen and idgrupo = p.idgrupo and idsocio = p.idsocio) > 0 
                            then 5
                            else (case when p.nacionalidad != 3 and (p.razon_social is NULL or p.razon_social = '') 
                            then 1
                            when p.nacionalidad != 3 and
                            (p.razon_social is not NULL and p.razon_social != '')
                            then 2
                            when p.nacionalidad  = 3 and (p.razon_social is NULL or p.razon_social = '') 
                            then 3
                            when p.nacionalidad  = 3 and (p.razon_social is not NULL and p.razon_social != '') 
                            then 4
                      end)
                    end)
                  end) as tipoc, -----DATO 5
                          (case
                            ...
                            when p.nivel_riesgo = 2 then 5
                            -----EL 3 ES RIESGO MEDIO NO APLICA PARA TODAS LAS CAJAS
...
create type numero_operaciones_producto_servicio_por_canal_envio as (
    ...
    clave_entidad                       text,
    producto_servicio                   integer,
    tipo_canal                          integer,---Validar si la Caja tiene transferencias
    operacion_entrada_salida            integer,
...
*/
    ...
    (select canal,count(*) numero_operaciones,sum(monto_operaciones) monto_operaciones,flujo_entrada_salida 
                 from (select (case when ad.idtipo = 3 and ad.cargoabono=0 and (ad.concepto like 'Transferencia de banca movil_%' 
                                                            or ad.concepto like 'Transferencia de banca movil_ Ref:%') then 5 /*Banca Móvil*/
                       ...
                       select (case when rc.idtipo = 3 and rc.cargoabono=1 and (rc.concepto like 'Transferencia de banca movil_%' 
                                                                                or rc.concepto like 'Transferencia de banca movil_ Ref:%') then 5 /*Banca Móvil*/
                                                        else 1 /*Ventanilla (sucursales, agencias u oficinas)*/ end) as canal,
                                                  -- GUS (round)
