...
begin
 ...
 ----TABLA FISICA PARA CONSULTAS CUESTIONARIO A1
...
for r_paso in select * from
        ...
        e.idestado as enti_fede_residencia, ----------DATO3
        (case 
...
-----SE AGREGO JOIN CON TRABAJOS PARA SU GENERACION EN LAS BASES DIC 2025 DE LAS COOPERATIVAS
        inner  join trabajo tr on ax.idorigen = tr.idorigen and ax.idgrupo = tr.idgrupo and ax.idsocio = tr.idsocio AND tr.consecutivo = 1
...
r.pais_nacionalidad                       :=trim(to_char(r_paso.nacion::integer,'99'));
r.pais_residencia                         :=trim(to_char(r_paso.nacion::integer,'999'));
...
-----SE AGREGO LA ACTVIVIDAD ECONOMICA PLD PARA SU GENERACION EN LAS BASES DIC 2025 DE LAS COOPERATIVAS
  ...
  execute 'copy (select fila from copiar order by id) to ''/tmp/formularioA1_con_encabezados_'||fecha||'.csv'' with csv DELIMITER ''-'' ';
  execute 'copy (select fila from copiar where id > 0 order by id) to ''/tmp/formularioA1_sin_encabezados_'||fecha||'.csv'' with csv DELIMITER ''-'' ';
