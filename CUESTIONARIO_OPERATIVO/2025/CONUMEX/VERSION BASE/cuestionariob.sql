...
------------------------------------------------------
  ...
  insert
  into   copiar
  values (y,'ANIO|CLAVE DEL FORMULARIO|CLAVE DE LA ENTIDAD|TIPO DE PRODCUTO O SERVICIO UTILIZADO|TIPO CLIENTE O USUARIO|ACTIVIDAD ECONOMICA|TIPO MONEDA|TIPO DE INSTRUMENTO MONETARIO|OPERACION DE ENTRADA Y SALIDA|NUMERO DE OPERACIONES|MONTO DE LAS OPERACIONES');
...
*/
  ...
  loop
    ...
    if (r_prod.dato2 is not null and TRIM(r_prod.dato2) <> '' 
      ...
      execute 'copy (select fila from copiar order by id)              to ''/tmp/Bv2_con_encabezados_'||fecha||'.csv'' with csv DELIMITER ''|'' ';
      execute 'copy (select fila from copiar where id > 0 order by id) to ''/tmp/Bv2_sin_encabezados_'||fecha||'.csv'' with csv DELIMITER ''|'' ';
