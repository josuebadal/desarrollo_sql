...
for r_paso in select * from
			  (select tipo_clien_usua,clasi_grado_riesgo,enti_fede_residencia,sum(num_total_clien_usua) as num_total_clien_usua/*, nacion*/ from
			  (select
				...
				from personas p
				left join (select * from auxiliares where idproducto=101 and saldo=1500
						   union all
						   select * from auxiliares where idproducto=120 and saldo>0--select * from auxiliares_h where idproducto = 101 and saldo = 0 and to_char(date(fechaumi),'YYYY')::integer=amo and idgrupo = 10 and estatus = 3
						   ) as ax using (idorigen,idgrupo,idsocio)
				...
				group by p.nacionalidad,p.razon_social,p.nivel_riesgo,e.idestado/*,p.pais_nacimiento*/) ope
			  group by tipo_clien_usua,clasi_grado_riesgo,enti_fede_residencia/*, nacion*/) aa
loop
