...
for r_paso in select * from 
			  ...
			  (select 
				...
				(case when p.idgrupo = 10 then (case when p.nivel_riesgo = 1 then 1
													 when p.nivel_riesgo = 2 then 5
												else 3 end)
					  when p.idgrupo = 20 then (case when p.nivel_riesgo = 1 then 1
												else 3 end)
					  when p.idgrupo = 30 then (case when p.nivel_riesgo = 1 then 1
													 when p.nivel_riesgo = 2 then 3
													 when p.nivel_riesgo = 3 then 5
												else 3 end)
				else 3 end) as clasi_grado_riesgo,
				count(*) as num_total_clien_usua,
				e.idestado as enti_fede_residencia
				from personas p
				left join (select a.idorigen,a.idgrupo,a.idsocio 
						   from auxiliares a
						   where a.idorigenp>0 and a.idproducto in (select idproducto from productos where tipoproducto in (0,1,2,8)) and a.idauxiliar>0 and a.saldo>0
						   /*union
						   select a.idorigen,a.idgrupo,a.idsocio 
						   from personas a
						   inner join bitacora_afectaciones ba on (ba.referencia = (TRIM(TO_CHAR(a.idorigen,'099999'))||trim(to_char(a.idgrupo,'09'))||TRIM(TO_CHAR(a.idsocio,'099999'))))
						   where to_char(date(ba.fecha),'YYYY')::integer=amo and ba.modulo = 'OP31' 
						   and ba.detalle like '%estatus:t@f%'*/) as ax using (idorigen,idgrupo,idsocio)
				...
				where ax.idsocio is not null and p.idgrupo in (10,20,30)
				group by p.nacionalidad,p.razon_social,p.nivel_riesgo,e.idestado,p.idgrupo) ope
			  group by tipo_clien_usua,clasi_grado_riesgo,enti_fede_residencia) aa
loop

/*r.periodo                                 :='2020';
r.clave_del_Formulario                    :='A1';
...
r.pais_nacionalidad                       :=160;
r.pais_residencia                         :=160;
r.entidad_federativa_residencia           :=trim(to_char(r_paso.enti_fede_residencia,'99'));
r.numero_total_clientes_o_usuarios        :=trim(to_char(r_paso.num_total_clien_usua,'99999999999999999'));*/
