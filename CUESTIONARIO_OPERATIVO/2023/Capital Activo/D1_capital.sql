...
create or replace function numero_operaciones_producto_por_canal (integer,integer)
	...
	(select canal,count(*) numero_operaciones,sum(monto_operaciones) monto_operaciones, flujo_entrada_salida
				 from (select (case when ad.idtipo=1 and po.concepto like '%***( EFECTIVO )***%' then 1 /*Efectivo*/
				 				      when ad.idtipo=2 then 3	/*Cheques Nominativo o al Portador girado en territorio nacional*/
				 					  when ad.idtipo=1 and po.concepto like '%***( SPEI )***%' then 3 /*Transferencias Electronicas de Fondos*/
				 				else 3 /*Resto idtipo=3*/ end) as canal,
