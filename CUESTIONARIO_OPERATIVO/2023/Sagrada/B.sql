...
numero_operaciones_producto_servicio (integer,integer)returns setof numero_operaciones_producto_servicio_por_moneda_instmonetario as $$
declare
...
begin
  ...
  loop
    ...
    if (r_prod.dato2 is not null and TRIM(r_prod.dato2) <> '') then
      for r_paso ------- FOR (B)
      in  select   insm,
                   sum(entradas_monto  + salidas_monto)  as monto_entr_sali,
                   sum(entradas_numero + salidas_numero) as num_entr_sali,
                   tipo_oper 
          from     (select case when cargoabono = 0
                                then monto_operaciones
                                else 0
                           end salidas_monto,
                            case when cargoabono = 1
                                then monto_operaciones
                                else 0
                           end entradas_monto,
                            case when cargoabono = 0
                                then numero_operaciones
                                else 0
                           end salidas_numero,
                            case when cargoabono = 1
                                then numero_operaciones
