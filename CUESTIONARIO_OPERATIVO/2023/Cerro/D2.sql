...
create or replace function numero_clientes_aperturaron_cuenta_por_canal (integer,integer)
	...
	select into r_paso sum(ts.total) as total_cli
			...
			from auxiliares a
			 where (a.idproducto = 101 and a.saldo >= 2000.0)  and a.fechaactivacion between '01/01/2023' and '31/12/2023') as ts;
	...
	execute 'copy (select fila from copiar order by id) to ''/tmp/D2_con_encabezados_'||fecha||'.txt'''; 
	execute 'copy (select fila from copiar where id > 0 order by id) to ''/tmp/D2_sin_encabezados_'||fecha||'.txt''';
