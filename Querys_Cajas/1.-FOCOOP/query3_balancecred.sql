...
COALESCE((SELECT bc.fechacierre from balancecred bc
        WHERE UPPER(bc.cartera) = 'V' 
        AND UPPER(bc.carteraant) = 'C'
        ),'01/01/1900') as "fecha_pg_sostenido",
...
split_part(numero_renovados_reestructurados_focoop(a.idorigenp,a.idproducto,a.idauxiliar),'|',1)::int4 AS "veces_reno_rees",
COALESCE((
...
(SELECT MAX(vd.fecha::date) 
    ...
    FROM auxiliares a
        ...
        INNER JOIN balancecred bc
                ON a.idorigenp = bc.idorigenp 
                AND a.idproducto = bc.idproducto 
                AND a.idauxiliar = bc.idauxiliar 
