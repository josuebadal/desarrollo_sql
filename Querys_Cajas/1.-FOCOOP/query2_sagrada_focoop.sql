...
DROP TYPE public.tipo_reg_453_focoop cascade;

CREATE TYPE public.tipo_reg_453_focoop AS (
...
-- DROP FUNCTION public.reg_453_focoop();

CREATE OR REPLACE FUNCTION public.reg_453_focoop()
 RETURNS SETOF tipo_reg_453_focoop
 LANGUAGE plpgsql
AS $function$
DECLARE
...
::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::*/
drop table if exists castigos_cnbv_reporte_focoop;
create temp table castigos_cnbv_reporte_focoop(
    clave_subreporte text,
    p1 text,
    numerosocio text,
    p2 text,
...
insert into castigos_cnbv_reporte_focoop values(
'0453',
'|',
...
'|',
 ...
 '000000000000000000.00',
  ...
  (select coalesce(to_char(max(fecha),'YYYY-MM-DD'),'ND') from servicios_d where idorigen = substr(r_entregar.numerosocio,0,15)::integer and idgrupo = substr(r_entregar.numerosocio,15,2)::integer and idsocio= substr(r_entregar.numerosocio,17,6)::integer and idproducto in (538, 539 , 561)),
  '|',

 (select coalesce(sum(monto),0) from servicios_d where idorigen = substr(r_entregar.numerosocio,0,15)::integer and idgrupo = substr(r_entregar.numerosocio,15,2)::integer and idsocio= substr(r_entregar.numerosocio,17,6)::integer and idproducto in (538, 539 , 561)),
 '|',
 ...
 'ND',
  ...
  ( case when  (select coalesce(sum(monto),0) from servicios_d where idorigen = substr(r_entregar.numerosocio,0,15)::integer and idgrupo = substr(r_entregar.numerosocio,15,2)::integer and idsocio= substr(r_entregar.numerosocio,17,6)::integer and idproducto in (538, 539 , 561)) = 0 then 'ND'  else 'En efectivo' end),
...
for registros in select * from castigos_cnbv_reporte_focoop loop
 ...
 r.nombre_administrativo_funcionario_autorizo       := 'ND';
 r.p39                                              := '|';
 r.cargo_administrativo_funcionario_autorizo        := 'ND';
 r.p40                                              := '|';


 r.fecha_recuperacion                               :=registros.fecha_recuperacion;                   
 r.p41                                              := '|'; 
 r.monto_recuperado                                 :=registros.monto_recuperado;                     
 r.p42                                              := '|';
 
 r.convenio_adeudo                                  := 'ND';   
 r.p43                                              := '|';
 r.tipo_recuperacion                                := registros.tipo_recuperacion;
 r.p44                                              := '|';
 r.tipo_adjudicacion                                := 'ND';
...
--  execute 'copy (select * from castigos_cnbv_reporte_focoop) to ''/tmp/reg_453_cuentas_con_encabezados_'||fechita||'.csv'' with csv HEADER DELIMITER '';'' NULL '' '' ';
--  execute 'copy (select * from castigos_cnbv_reporte_focoop) to ''/tmp/reg_453_cuentas_sin_encabezados_'||fechita||'.csv'' with csv DELIMITER '';''  ';
