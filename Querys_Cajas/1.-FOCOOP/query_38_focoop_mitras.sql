...
IDTabla: claves_reg453
IDElemento: cuenta_castigo
Parametro1: 10601 (iniciales de la cuenta contable de castigos)

FECHAS SOLICITADAS POR LA CAJA 1 de enero 2023 al 30 de septiembre de 2025,
FORMATO 'AAAA-MM-DD'
fecha_inicial    date := '2023-01-01';
fecha_Final      date := '2025-09-30';
...
repreg2_desagregado_de_cartera_de_credito_castigada_c0453() returns integer AS $$
DECLARE
x_cont integer;
BEGIN
  select
...
CREATE OR REPLACE FUNCTION query_focoop()
...
::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::*/
  ...
  LOOP
    ...
    SELECT INTO r_total
                coalesce(SUM(monto),0) as monto, coalesce(SUM(montoio),0) as montoio,
                coalesce(SUM(montoim),0) as montoim, coalesce(SUM(montoiva),0) as montoiva,
                coalesce(SUM(montoivaim),0) as montoivaim, COUNT(*) as numero_registros, MAX(monto) as max_monto
    ...
    LOOP
      ...
      ELSIF (r_castigos.monto = (r_total.monto + r_total.montoio + r_total.montoiva + r_total.montoim + r_total.montoivaim)) THEN
...
/*
      ...
      RAISE NOTICE 'Diferencia: %', x_diferencia;
*/
  if (r_auxi.idorigenp, r_auxi.idproducto, r_auxi.idauxiliar) = (30160,32002,30) then
    RAISE NOTICE '>> %-%-% >>>> GUS >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>< monto de auxiliares: %', r_auxi.idorigenp, r_auxi.idproducto, r_auxi.idauxiliar, r_auxi.monto;
  end if;
...
create table prestamos_condonaciones (
  idorigen      integer,
  idgrupo       integer,
  idsocio       integer,
  idorigenp     integer,
...
for r_condonaciones in
       select a.idorigen,a.idgrupo,a.idsocio,ad.idorigenp,ad.idproducto,ad.idauxiliar, ad.fecha::date,ad.cargoabono,ad.monto,
              ...
              ad.montoivaim ,ad.efectivo,ad.diasvencidos,ad.montovencido,ad.ticket ,ad.montoidnc,ad.montoieco,ad.montoidncm,ad.montoiecom
...
loop
  ...
  if (r_ogs_opa.idorigenp, r_ogs_opa.idproducto, r_ogs_opa.idauxiliar) = (30160,32002,30) then
    RAISE NOTICE '>> %-%-% >>>> GUS >> ZZZ >>>>>>>>>>>>>>>>>>>>>>>>>>>>>><', r_ogs_opa.idorigenp, r_ogs_opa.idproducto, r_ogs_opa.idauxiliar;
  end if;
...
(SELECT (c.monto)
 ...
 as castigo
    ...
    (select ((case when ct.monto_real_castigo>0 then ct.monto_real_castigo else 0 end)
        +(au.montoio+au.montoim-au.montoieco-au.montoiecom))
     from prestamos_condonaciones au
     left join castigos ct using(idorigen,idgrupo,idsocio,idorigenp,idproducto,idauxiliar)
     where (au.idorigen,au.idgrupo,au.idsocio,au.idorigenp,au.idproducto,au.idauxiliar) 
         =
           (gen.idorigen,gen.idgrupo,gen.idsocio,gen.idorigenp,gen.idproducto,gen.idauxiliar) limit 1) as montodeestimacionespreventivas,
...
/*
  ...
  coalesce(trim(to_char((select am.vence from amortizaciones_h am
                         where am.idorigenp = f.idorigenp and am.idproducto = f.idproducto and am.idauxiliar = f.idauxiliar and am.diasvencidos > 0
                         union
                         select am.vence from amortizaciones am
                         where am.idorigenp = f.idorigenp and am.idproducto = f.idproducto and am.idauxiliar = f.idauxiliar and am.diasvencidos > 0
                         union
                         select am.vence from amortizaciones_h am
                         where am.idorigenp = f.idorigenp and am.idproducto = f.idproducto and am.idauxiliar = f.idauxiliar and
                               am.vence >= date((select date(max(ad.fecha)) from v_auxiliares_d ad
                                                 where ad.cargoabono=1 and date(ad.fecha) < date(f.fechacastigo) and (ad.idorigenp,ad.idproducto,ad.idauxiliar)=(f.idorigenp,f.idproducto,f.idauxiliar)))
                         order by vence limit 1),'yyyy-mm-dd')
                ),'9999-12-31') as fecha_primera_amortizacion_no_cubierta,-------Primera Amortizacion no cubierta
...
(case when r_entregar.montodeestimacionespreventivas is null then '00000000000000000.00' else trim(to_char(r_entregar.montodeestimacionespreventivas ,'09999999999999999.99')) end),
...
select into fechita to_char(CURRENT_DATE,'dd|mm|yyyy');
  ...
  execute 'copy (select * from castigos_cnbv_reporte) to ''/tmp/query_38_con_encabezados_'||fechita||'.csv'' with csv HEADER DELIMITER '';'' NULL '' '' ';
  execute 'copy (select * from castigos_cnbv_reporte) to ''/tmp/query_38_sin_encabezados_'||fechita||'.csv'' with csv DELIMITER '';''  ';
...
/*
Comparacion de inicial de cadena = case when substring(trim(to_char(registros.Montodeestimacionespreventivas,'0099999999999.99')),1,1) = '-' then trim(to_char(registros.Montodeestimacionespreventivas,'099999999999.99')) else trim(to_char(registros.Montodeestimacionespreventivas,'0099999999999.99')) end;
...
--select * from reg_453(@@Fecha_ini:|f|01/01/2019@@, @@Fecha_Final:|f|31/01/2019@@);

CREATE OR REPLACE FUNCTION reg_453(date,date)
RETURNS setof tipo_reg_453 AS $$

DECLARE

fecha_inicial  alias for $1;
...
BEGIN
   ---- modificacion de leon franco por periodo 30_05_2019   fredy se quito este apartado y se le puso para que seleccionara rango de fechas
-- select
-- into   fecha_final, fecha_inicial
--        fecha,
--        (01||to_char(fecha,'/mm/yyyy'))::date
-- from   (select distinct date(fechatrabajo) as fecha
--         from            origenes) as x;

-- fecha_final   := date('28/04/2019');
-- fecha_inicial := date('01/04/2016');
...
from(SELECT c1.fecha,c1.idorigen, c1.idgrupo, c1.idsocio,
 ...
 COALESCE((select SUM(au.montoidnc)
           from   prestamos_condonaciones au
           where  (au.idorigen,au.idgrupo,au.idsocio,au.idorigenp,au.idproducto,au.idauxiliar) =
                  (gen.idorigen,gen.idgrupo,gen.idsocio,gen.idorigenp,gen.idproducto,gen.idauxiliar )
           limit  1),0)
 +
 COALESCE((select SUM(c2.castigo)
           from   (select a.idorigen,a.idgrupo, a.idsocio, a.idorigenp, a.idproducto, a.idauxiliar, a.castigo
                   from   (SELECT c1.idorigen, c1.idgrupo, c1.idsocio, c1.idorigenp, c1.idproducto, c1.idauxiliar,
                                  c1.idorigenc, c1.periodo, c1.idtipo, c1.idpoliza, c1.monto,
                                  (case when (select count(*)
                                              from   castigos c3
                                              where  (c3.idorigen,c3.idgrupo,c3.idsocio,c3.idorigenc, c3.periodo, c3.idtipo, c3.idpoliza)=
                                                       (c1.idorigen,c1.idgrupo,c1.idsocio,c1.idorigenc, c1.periodo, c1.idtipo, c1.idpoliza) and
                                                     c3.idproducto >= 30000 AND c3.idproducto <= 39999) > 1
                                        then round((SELECT (c.monto)
                                                    FROM   castigos AS c
                                                    WHERE  c.idorigenc = c1.idorigenc AND c.periodo = c1.periodo AND c.idtipo = c1.idtipo AND
                                                           c.idpoliza = c1.idpoliza AND c.idorigen = c1.idorigen AND c.idgrupo = c1.idgrupo AND
                                                           c.idsocio = c1.idsocio AND c.idproducto = 60001 ) /
                                                   (select count(*)
                                                    from   castigos c3
                                                    where  (c3.idorigen,c3.idgrupo,c3.idsocio,c3.idorigenc, c3.periodo, c3.idtipo, c3.idpoliza) =
                                                             (c1.idorigen,c1.idgrupo,c1.idsocio,c1.idorigenc, c1.periodo, c1.idtipo, c1.idpoliza) and
                                                            c3.idproducto >= 30000 AND c3.idproducto <= 39999),3)
                                        else (SELECT (c.monto)
                                              FROM   castigos AS c
                                              WHERE  c.idorigenc = c1.idorigenc AND c.periodo = c1.periodo AND c.idtipo = c1.idtipo AND
                                                     c.idpoliza = c1.idpoliza AND c.idorigen = c1.idorigen AND c.idgrupo = c1.idgrupo AND
                           ...
                           FROM   castigos AS c1
                           WHERE  c1.idproducto >= 30000 AND c1.idproducto <= 39999 ) as a
                   where  a.castigo is not null) c2
           where  (c2.idorigen,c2.idgrupo,c2.idsocio,c2.idorigenp,c2.idproducto,c2.idauxiliar) =
                    (gen.idorigen,gen.idgrupo,gen.idsocio,gen.idorigenp,gen.idproducto,gen.idauxiliar)), 0) as Montodeestimacionespreventivas,
