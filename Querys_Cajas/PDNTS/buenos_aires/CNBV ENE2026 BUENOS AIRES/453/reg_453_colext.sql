...
---------- SE AGREG COLUMNA EXTRA ----------
 poliza         character varying(26)
);
...
create temp table final(
 ...
 fecha_sic                       text,
 tipocobranza                    text,
 --------- SE NECESITA AGREGAR EL NUMERO DE POLIZA POR CNBV---------
 poliza                          text);
...
COALESCE(
  (
    SELECT au.idorigenc || '-' || au.periodo || '-' || au.idtipo || '-' || au.idpoliza
    FROM castigos au
    WHERE (au.idorigen, au.idgrupo, au.idsocio, au.idorigenp, au.idproducto, au.idauxiliar) =
          (gen.idorigen, gen.idgrupo, gen.idsocio, gen.idorigenp, gen.idproducto, gen.idauxiliar)
  ...
  (
    SELECT au.idorigenc || '-' || au.periodo || '-' || au.idtipo || '-' || au.idpoliza
    FROM prestamos_recuperaciones au
    WHERE (au.idorigen, au.idgrupo, au.idsocio, au.idorigenp, au.idproducto, au.idauxiliar) =
          (gen.idorigen, gen.idgrupo, gen.idsocio, gen.idorigenp, gen.idproducto, gen.idauxiliar)
    ORDER BY au.fecha DESC
    LIMIT 1
...
r_entregar.fecha_sic,
 ...
 (case when r_entregar.montodegarantialiquida is null 
    then '000000000000000000.00' 
    else trim(to_char(r_entregar.montodegarantialiquida,'099999999999999999.99')) end),
...
return next r;
                ...
                RAISE NOTICE'|DATOS_REPORTE|%',     coalesce(registros.clave_subreporte::text,'')
                                                  ...
                                                  ||';'||coalesce(registros.poliza::text,'');
