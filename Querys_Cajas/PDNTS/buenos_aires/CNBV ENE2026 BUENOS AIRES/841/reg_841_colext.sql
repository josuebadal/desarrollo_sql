/*====================================================================================================
  ====================================================================================================*/
 ---               QUE SE OCUPA PARA GENERAR EL REGULATORIO 841
 --- 1- Tener la base del ultimo mes que se genero el reg 841 trimestral
 --  2- Cargar las polizas extempraneas
 --  3- Cargar en esa base el Script para corregir las garantias_liquidas, creado por Eliseo.
 --  4- Cargar el Scrip shell que importa saldos en esa base, creado por Gustavo


 --******* Despues en la base actual donde se ejecutara el regulatorio 841 trimestral*******

 --  *******************Cargaremos las siguientes funciones***********************
--- 1- Tener cargada las polizas fin de mes
 -- 2- Cargar de nuevo el Script para corregir las garantias_liquidas, creado por Eliseo.
 -- 3- Tener configurada la tabla idtabla='prod_garantia_841', con los nombres que registraron sus productos
 ----Ejemplo:
  /*    idtabla      | idelemento | nombre | dato1 | dato2 | dato3 | dato4 | dato5 | tipo 
    -------------------+------------+--------+-------+-------+-------+-------+-------+------
     prod_garantia_841 | 101        |        | PS    |       |       |       |       |    0
     prod_garantia_841 | 103        |        | CA    |       |       |       |       |    0
     prod_garantia_841 | 110        |        | AH    |       |       |       |       |    0
     prod_garantia_841 | 111        |        | AN    |       |       |       |       |    0
     prod_garantia_841 | 114        |        | AE    |       |       |       |       |    0
     prod_garantia_841 | 115        |        | AD    |       |       |       |       |    0
     prod_garantia_841 | 116        |        | HE    |       |       |       |       |    0
     prod_garantia_841 | 117        |        | EM    |       |       |       |       |    0
     prod_garantia_841 | 118        |        | PA    |       |       |       |       |    0
     prod_garantia_841 | 120        |        | AM    |       |       |       |       |    0
     prod_garantia_841 | 130        |        | CC    |       |       |       |       |    0
     prod_garantia_841 | 200        |        | D1    |       |       |       |       |    0
     prod_garantia_841 | 201        |        | D2    |       |       |       |       |    0
     prod_garantia_841 | 202        |        | D3    |       |       |       |       |    0
     prod_garantia_841 | g110       |        | GA    | 1     |       |       |       |    0
     prod_garantia_841 | g200       |        | G1    | 1     |       |       |       |    0
     prod_garantia_841 | g201       |        | G2    | 1     |       |       |       |    0
     prod_garantia_841 | g202       |        | G3    | 1     |       |       |       |    0*/
...
RETORNO:
        ...
        reg_841_con_encabezados_fecha_hoy.csv
        reg_841_sin_encabezados_fecha_hoy.csv


las tablas de configuracion de la tabla son los siguientes


IdTabla: claves_reg841
IDElemento: productos_captacion
Parametro2: (lista de productos separados por pipas)

IdTabla: claves_reg841
IDElemento: certificados_excedentes
Paramentro1: (producto de certificado excedente)
Parametro2: (monto de certificado excedente)
  

IdTabla: claves_reg841
IDElemento: certificados_ordinarios
Paramentro1: (producto de certificado ordinario)
Parametro2: (monto de certificado ordinario)

IdTabla: claves_reg841
IDElemento: pago_interes
Parametro2: (lista de productos separados por pipas)
...
DROP TYPE IF EXISTS tipo_reg_841 CASCADE;
CREATE TYPE tipo_reg_841 AS (
 formulario                                          text ,              
...
CREATE OR REPLACE FUNCTION reg_841()
  RETURNS SETOF tipo_reg_841 AS
$$

DECLARE
query                   text;
r                       tipo_reg_841%rowtype;
rec                     record;
x                       integer;
px1                     varchar;
px2                     varchar;
...
(case when a.idproducto = (select dato1::integer from tablas where idtabla=''claves_reg841'' and idelemento like ''certificados_ordinarios%'')
            then ''000''
else ''001'' end) AS "TIPO_DE_APERTURA_DE_CUENTA",--------------------------------------------------------------------------------------37

    a.idorigenp,a.idproducto,a.idauxiliar,a.idorigen,a.idgrupo,a.idsocio,
-------------------------------------INICIA COLUMNAS EXTRA QUE SOLICITAN CNBV
...
(CASE
      WHEN EXISTS (SELECT 1 FROM sopar sp 
            ...
            AND sp.idusuario  = 999 AND sp.tipo = ''lista_personas_bloqueadas_cnbv''
      ) THEN ''bloqueado''
...
numero_reinversiones_focoop(a.idorigenp, a.idproducto, a.idauxiliar) as "numero_reinversiones",
    ...
    a.plazo as aux_plazo, a.fechaactivacion as aux_fechaactivacion,
...
FROM (select * from v_auxiliares
      where estatus = 2 /*and idproducto in (select * from prd_captacion)*/
        and idproducto in (select (case when idproducto = (select dato1::integer from tablas where idtabla=''claves_reg841'' and idelemento like ''certificados_ordinarios%'') and saldo > 0
                                        then idproducto
                                        when idproducto != (select dato1::integer from tablas where idtabla=''claves_reg841'' and idelemento like ''certificados_ordinarios%'')
                                        then idproducto
                                   end)
                           from prd_captacion)
      union
      ...
      where estatus = 3
        and to_char(fechauma,''yyyymm'')::integer between (select distinct to_char(date(fechatrabajo),''yyyymm'')::integer-2 from origenes)
                                                          and
 ...
 LEFT JOIN trabajo tr ON tr.idorigen = p.idorigen and tr.idgrupo = p.idgrupo 
     and tr.idsocio = p.idsocio AND tr.consecutivo = 1
 LEFT JOIN actividades_economicas acte on tr.actividad_economica =  acte.id_actividad
 left join (select  * 
            from    (select   ROW_NUMBER() over(PARTITION BY origenp_ah,producto_ah,auxiliar_ah
            ...
            order by origenp_ah,producto_ah,auxiliar_ah,indice) as garan
              on (garan.origenp_ah = a.idorigenp and garan.producto_ah = a.idproducto  and garan.auxiliar_ah = a.idauxiliar)
where p.nombre !=''(*)'' 
  and p.nombre !=''*'' 
  and p.nombre !=''***''
  and p.nombre !=''X''
...
---Se modifico para que solo en la parte numero_de_contrato y numero_de_cuenta aparezca 0 cuando sea parte social o certificado. fecha: 26/04/2022
  ...
  loop
    ...
    x_tipo_producto   := case when coalesce(rec.origen_pres,0) > 0 and rec.pr_tipoproducto = 0
                              then '006'
                              when (substr(rec.idorigen::text,1,3) = '310' or (substr(rec.idorigen::text,1,3) = '104'))
                                   and rec.pr_tipoproducto in (1,8) -----------------------------------------------------San Isidro y Nueve de Agosto utiliza el tipo_producto 007 para las inversiones
                              then '007'
                              when coalesce(rec.origen_pres,0) > 0 and rec.pr_tipoproducto in (1,8)
                              then '010'
                              when coalesce(rec.origen_pres,0) = 0 and rec.pr_tipoproducto in (1,8)
                              then '009'
    ...
    x_fecha_venc  := case when (substr(rec.idorigen::text,1,3) = '305' or substr(rec.idorigen::text,1,3) = '310') and rec."TIPO_DE_PRODUCTO"::integer = 5 and coalesce(rec.origen_pres,0) > 0
...
RAISE NOTICE'|DATOS_REPORTE_COMPROBACION|%',coalesce(r.formulario::text,'')
  ...
  */                                 
  ...
  execute 'copy (select fila from copiar where SAI_TOKEN(4,fila,''|'')!=''X'' order by id) to ''/tmp/reg_841_ce_buenos_cnbv_'||fecha||'_'||origen||'.csv''  ';
  execute 'copy (select fila from copiar where id > 0 order by id) to ''/tmp/reg_841_se_buenos_cnbv_'||fecha||'_'||origen||'.csv''  ';
