...
DROP TYPE IF EXISTS tipo_reg_841_mensual CASCADE;
CREATE TYPE tipo_reg_841_mensual AS (
 formulario                                          text ,              
...
CREATE OR REPLACE FUNCTION reg_841_mensual()
  RETURNS SETOF tipo_reg_841_mensual AS
$$

DECLARE
query                   text;
r                       tipo_reg_841_mensual%rowtype;
rec                     record;
x                       integer;
px1                     varchar;
px2                     varchar;
...
DROP table IF EXISTS reg_841_mensual;
CREATE table reg_841_mensual (
 formulario                                          text ,              
 numero_de_identificacion                            text,                
 tipo_de_socio                                       text,             
 nombre_razon_o_denominacion_social                  text  ,                
...
(CASE WHEN idgrupo IN(10,20,25) AND  LENGTH(p.rfc)=10 THEN RPAD(p.rfc,13,''_'') ELSE p.rfc END)as "RFC_DEL_SOCIO",--------------9
...
--                          WHEN pr.cuentaaplica LIKE ''201020201%'' and g_idorigenp is NULL      THEN ''009''
--                          WHEN pr.cuentaaplica LIKE ''2010202%''   and g_idorigenp is not NULL  THEN ''010'' 
     ...
     where idorigenp=a.idorigenp and idproducto=a.idproducto and idauxiliar=a.idauxiliar order by fecha desc limit 1)*/
          ...
          ((case when a.idproducto in (select dato1::integer from tablas where idtabla=''claves_reg841'' and idelemento like ''certificados%'')
            ...
            case when a.idproducto in (select dato1::integer from tablas where idtabla=''claves_reg841'' and idelemento like ''certificados%'')
                 ...
                 coalesce((select sum(monto) from v_auxiliares_d ad where ad.cargoabono=0 and ad.periodo::integer = (select distinct to_char(date(fechatrabajo),''yyyymm'')::integer from origenes) and
          ...
          (case when a.idproducto in (select dato1::integer from tablas where idtabla=''claves_reg841'' and idelemento like ''certificados%'')
                ...
                coalesce((select sum(monto) from v_auxiliares_d ad where ad.cargoabono=1 and ad.periodo::integer = (select distinct to_char(date(fechatrabajo),''yyyymm'')::integer from origenes) and
         ...
         (case when a.idproducto in (select dato1::integer from tablas where idtabla=''claves_reg841'' and idelemento like ''certificados%'')
               then 0.00
                    ...
                    coalesce((select sum(monto) from v_auxiliares_d ad
                              where ad.cargoabono=1 and ad.periodo::integer = (select distinct to_char(date(fechatrabajo),''yyyymm'')::integer from origenes) and 
                                    (ad.idorigenp,ad.idproducto,ad.idauxiliar) = (a.idorigenp,a.idproducto,a.idauxiliar)),0.00)
          end)
    ...
    when garan.producto_ah in (select * from prd_captacion) and g_idorigenp=0 then retiros
         ...
         (case when a.idproducto in (select dato1::integer from tablas where idtabla=''claves_reg841'' and idelemento like ''certificados%'')
               then 0.00
                ...
                coalesce((select sum(monto) from v_auxiliares_d ad
                          where ad.cargoabono=0 and ad.periodo::integer = (select distinct to_char(date(fechatrabajo),''yyyymm'')::integer from origenes) and 
                                (ad.idorigenp,ad.idproducto,ad.idauxiliar) = (a.idorigenp,a.idproducto,a.idauxiliar)),0.00)
          end)
...
(CASE
  WHEN EXISTS (
    ...
    JOIN actividades_economicas ae
      ON ae.id_actividad = tr.actividad_economica
    WHERE tr.idorigen = p.idorigen
      AND tr.idgrupo  = p.idgrupo
      AND tr.idsocio  = p.idsocio
      AND ae.idrecursivo BETWEEN 1 AND 67
  ) THEN ''Agricola''
  ...
  WHEN EXISTS (
    ...
    WHERE tr.idorigen = p.idorigen
      AND tr.idgrupo  = p.idgrupo
      AND tr.idsocio  = p.idsocio
      AND ae.idrecursivo BETWEEN 68 AND 134
  ) THEN ''Explotacion,Energia y Construccion''
  ...
  WHEN EXISTS (
    ...
    WHERE tr.idorigen = p.idorigen
      AND tr.idgrupo  = p.idgrupo
      AND tr.idsocio  = p.idsocio
      AND ae.idrecursivo BETWEEN 135 AND 260
  ) THEN ''Comercio''
  ...
  WHEN EXISTS (
    ...
    WHERE tr.idorigen = p.idorigen
      AND tr.idgrupo  = p.idgrupo
      AND tr.idsocio  = p.idsocio
      AND ae.idrecursivo BETWEEN 261 AND 662
  ) THEN ''Servicios''
...
FROM (select * from v_auxiliares
      ...
      where estatus = 3
        and to_char(fechauma,''yyyymm'')::integer = (select distinct to_char(date(fechatrabajo),''yyyymm'')::integer from origenes)
...
----TABLA FISICA REGULATORIO841_mensual
drop table if exists regulatorio841_mensual;
CREATE TABLE regulatorio841_mensual (
 formulario                                          text ,              
 numero_de_identificacion                            text  ,                
 tipo_de_socio                                       text ,           
 nombre_razon_o_denominacion_social                  text  ,                
...
---Se modifico para que solo en la parte numero_de_contrato y numero_de_cuenta aparezca 0 cuando sea parte social o certificado. fecha: 26/04/2022
  ...
  loop
    ...
    x_fecha_venc  := case when substr(rec.idorigen::text,1,3) = '305' and rec."TIPO_DE_PRODUCTO"::integer = 5 and coalesce(rec.origen_pres,0) > 0 ----------------------PARA FAMA
...
insert into regulatorio841_mensual values(
 r.formulario,
...
return next r;
  ...
  execute 'copy (select fila from copiar where SAI_TOKEN(4,fila,''|'')!=''X'' order by id) to ''/tmp/reg_841_con_encabezados_'||fecha||'_'||origen||'_'||'mensual'||'.csv''  ';
  execute 'copy (select fila from copiar where id > 0 order by id) to ''/tmp/reg_841_sin_encabezados_'||fecha||'_'||origen||'_'||'mensual'||'.csv''  ';
