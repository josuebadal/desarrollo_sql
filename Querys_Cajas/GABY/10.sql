...
a.plazo AS "plazo_pba",
---SE AGREGO ESTE DATO PARA VALIDAR DE DONDE NACIO eliminar al final
split_part(numero_renovados_reestructurados_focoop
(a.idorigenp,a.idproducto, a.idauxiliar),'|',1)::int4
AS "veces_reno_rees",
...
---se duplica el dato anterior ya que como esta pagado debe salir 0
--- en caso de algun error y estar activo se traera el saldo
...
---COMENTAN QUE SALEN DATOS VENCIDOS SE AGREGA EL TIPO 3
---NO SALIAN REGISTROS MAYORES A 89 DIAS
--VALIDAR SI DEBE APARECER EL TIPO 3
...
---AGREGAR UNA VALIDACION PARA CUANDO EL MISMO OPA PONER 0
--- VALIDAR CON GABY COMO HACERLO EN EL CASE
'1' AS "veces_reno_rees",
----COMENTAN QUE SALEN DATOS VACIOS
...
COALESCE((SELECT (CASE
        ...
        WHERE vd.idorigenp  = rp.idorigenp
           ...
           AND vd.monto > 0
           AND vd.montoio>0
           ORDER BY vd.fecha DESC LIMIT 1),'Tipo1') AS "tipo_cartera_ori",
 
---LA EVIDENCIA DE PAGO SOSTENIDO ESTA EN REFERENCIASP
(CASE
    WHEN a.tipoprestamo = 0 THEN 'vigente'
