...
begin
  ...
  insert
  into   copiar
  values (y,'anio;clave_formulario;clave_entidad;producto_servicio;tipo_cliente_usuario;act_eco_pld;tipo_moneda;tipo_instrumento_monetario;operac_entr_salida;numero_operaciones;monto_operaciones');
  y := 1;
  ...
  ------------------------------------------------------------
  -- TABLA PARA DETERMINAR LOS SOCIOS PPE 
  ------------------------------------------------------------
  ...
  create temp table temp_peps as
    ...
    from (select i.idorigen,i.idgrupo,i.idsocio,a.idorigenp,a.idproducto,a.idauxiliar
          ...
          where lista = 'PEP' and c.fecha_cargo_ini is not NULL and c.fecha_cargo_ini != ''
         UNION ALL
          select i.idorigen,i.idgrupo,i.idsocio, a.idorigenp, a.idproducto, a.idauxiliar
          from peps_qeq_identificado_web as i
               inner join peps_qeq_catalogo_web as c using (id_persona)
               inner join auxiliares as a using(idorigen,idgrupo,idsocio)
          where lista = 'PEP') as te;
...
*/
  ...
  loop
    ...
    if (r_prod.dato2 is not null and TRIM(r_prod.dato2) <> '' or r_prod.idelemento::integer=27) then --****Agregué el or para que permita ingresar el prod 27 (ordenes de pagos nacionales)****
      ...
      for r_paso ------- FOR (B)
      in select
      insm, tipo_oper,tipo_clien_usua ,act_eco_pld, sum(monto_entr_sali) as monto_entr_sali, sum(num_entr_sali)   as num_entr_sali
      from (
...
-- ESTE ES EL NIVEL NUEVO (CNBV)
group by insm, tipo_oper, tipo_clien_usua, act_eco_pld
order by insm, tipo_oper 
