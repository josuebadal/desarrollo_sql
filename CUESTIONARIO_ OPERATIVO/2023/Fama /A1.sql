...
for r_paso in select * from
			  ...
			  (select 
				...
				from personas p
				left join (select * from auxiliares where idproducto=101 and saldo=15000 
							union all 
...
r.entidad_federativa_residencia           :=trim(to_char(case when r_paso.enti_fede_residencia=7 then 5 else r_paso.enti_fede_residencia end,'99'));
